...
def find_elec_prop(population, poly_size, smiles_list, regr=None, machlearn='no'):
    '''
    ...
    smiles_list: list
        list of all possible monomer SMILES
    regr: object
        trained ML regressor
    machlearn: str
        sets machine learning on or off
    ...
    if machlearn != 'yes-solo':
        for polymer in population:
            if regr != None:
                # run ML if turned on
                rep = ecfp(polymer, poly_size, smiles_list)
                pred_polar = regr.predict(rep)
                poly_pred_polar_list.append(pred_polar)
                poly_ecfp.append(rep)
        ...
        for polymer in population:
            rep = ecfp(polymer, poly_size, smiles_list)
            pred_polar = regr.predict(rep)
            poly_polar_list.append(pred_polar)
            poly_pred_polar_list.append(pred_polar)
            poly_ecfp.append(rep)
...
def init_gen(pop_size, poly_size, num_mono_species, opt_property, smiles_list, machlearn, regr=None):
    '''
    ...
    params: list (specific format)
        [pop_size, poly_size, num_mono_species, opt_property, smiles_list, sequence_list, mono_list, population, poly_property_list, n, gen_counter, spear_counter, prop_value_counter, machlearn]
    regr: object
        trained ML regressor
    '''
    ...
    # Update the ML w/ new polymers
    if regr != None and machlearn == 'yes':
        regr = reteach(poly_ecfp_list, pred_polar_list, regr)
    ...
    pred_file = open('gens_pred.txt', 'w+')
    if opt_property == 'dip':
    ...
    for prediction in pred_polar_list:
    ...
    shutil.copy('gens_pred.txt', 'gens_pred_copy.txt')
    if opt_property == 'dip':
...
def next_gen(params, regr=None):
    ...
    pred_file = open('gens_pred.txt', 'a+')
    if opt_property == 'dip':
    ...
    params = [pop_size, poly_size, num_mono_species, opt_property, smiles_list, sequence_list,
              mono_list, population, poly_property_list, n, gen_counter, spear_counter, prop_value_counter, machlearn]
...
def main():
    ...
    # turn on ml options: yes, yes-solo, no
    machlearn = 'yes-solo'

    if machlearn == 'yes' or machlearn == 'yes-solo':
        regr = load_regressor('../input_files/ecfp_polar_brr.pkl')
    ...
    if restart == 'y':
        ...
        if regr != None:
            params, regr = init_gen(pop_size, poly_size, num_mono_species, opt_property, smiles_list, machlearn, regr)
        else:
            params = init_gen(pop_size, poly_size, num_mono_species, opt_property, smiles_list, machlearn, regr)
    ...
    for x in range(1):
        ...
        if regr != None:
            params, regr = next_gen(params, regr)
            if machlearn == 'yes':
                # this is to store updated regressor
                store_regressor('../input_files/ecfp_polar_brr_updated.pkl')
        else:
